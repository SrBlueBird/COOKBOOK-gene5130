...
```{r setup}
    ...
    knitr::opts_knit$set(root.dir = normalizePath("~/Rclass/week5")) 
```
...
```{r}
goodSamplesGenes(transposeBigData(counts))
```
...
```
```{r}
goodcounts <- counts[gsG$goodGenes,]
```
...
#### REMOVE THIS 
#prcomp$x
ggplot(data = prcomp$x, aes(x = PC1, y = PC2)) + geom_point() + geom_text(label = rownames(prcomp$x))
```
...
phenotype <- metadata[!(rownames(metadata) %in% c('GSM4615000','GSM4614995','GSM4614993')),]
temp_rename <- rename
rename <- dplyr::rename
...
new_pheno <- phenotype %>%
  ...
  rename(geo_location = geographical.location.ch1 , severity = severity.ch1 , days_post_symptom_onset = days_post_symptom_onset.ch1 )
...
```{r}
library(DESeq2)
dds <- DESeqDataSetFromMatrix(countData = realygoodcounts, 
                              colData = new_pheno,
                              design = ~ 1)
...
#traits <- new_pheno_name %>%
 # mutate(disease_state_bin = ifelse(grepl('COVID', disease_state),1,0))df %>%
  #mutate(disease_state_bin = ifelse(grepl('ICU', disease_state),1,0))
  #mutate(Mod_bin = ifelse(grepl("Moderate", disease_state),1,0))
  #mutate(Severe_bin = ifelse(grepl("Severe", disease_state),1,0))\
  #mutate(Conv_bin = ifelse(grepl("Convalescent", disease_state),1,0))
  #dplyr :: select(8:12)
```
...
```{r}
library(readr)
Correlations_1 <- read_csv("Correlations-1.csv")
pvalues <- read.csv('Pvalues-1.csv', header = TRUE, row.names = 1)
```
```{r}
